...
public class PagerSlidingTabStrip extends HorizontalScrollView implements View.OnClickListener {
    private Drawable slidingStrokeDrawable; //整个滑块的shape背景
    private Context context;
    private int slidingStrokeColor; //滑块之间的分割线
    ...
    private Drawable slidingLeftDrawable;    //左滑块
    private Drawable slidingMiddleDrawable;    //中间滑块
    private Drawable slidingRightDrawable;    //右滑块
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            if (attrsTypedArray != null) {
                ...
                slidingStrokeDrawable = attrsTypedArray.getDrawable(R.styleable.PagerSlidingTabStrip_slidingStrokeShape);
                slidingLeftDrawable = attrsTypedArray.getDrawable(R.styleable.PagerSlidingTabStrip_sliding_left);
                slidingMiddleDrawable = attrsTypedArray.getDrawable(R.styleable.PagerSlidingTabStrip_sliding_middle);
                slidingRightDrawable = attrsTypedArray.getDrawable(R.styleable.PagerSlidingTabStrip_sliding_right);
                disableViewPager = attrsTypedArray.getBoolean(R.styleable.PagerSlidingTabStrip_disableViewPager, false);
                slidingStrokeColor = attrsTypedArray.getColor(R.styleable.PagerSlidingTabStrip_slidingStrokeColor, Color.parseColor("#26a16e"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.tabsLayout != null){
            this.tabsLayout.setBackground(slidingStrokeDrawable);
        }
        if (tabsLayout != null && tabsLayout.getChildCount() > 0) {
            View childView = tabsLayout.getChildAt(0);
            int width = childView.getWidth();
            int height = childView.getHeight();
            float lineWidth = DensityUtil.dip2px(context, (float) 1.0);
            Paint linePaint = new Paint();
            linePaint.setColor(slidingStrokeColor);
            for (int i = 0; i < tabsLayout.getChildCount() - 1; i++) {
                canvas.drawRect(width * (i + 1) - lineWidth / 2, 0, width * (i + 1) + lineWidth / 2, height, linePaint);
            }
            ...
            if (currentTab != null) {
                ...
                float slidingBlockTop = currentTab.getTop();
                float slidingBlockBottom = currentTab.getBottom();
...
//                    Bitmap bitmap = drawableToBitmap(slidingLeftDrawable);
                    RectF dst = new RectF(slidingBlockLeft, slidingBlockTop, slidingBlockRight, slidingBlockBottom);
//                    canvas.drawBitmap(bitmap, null, dst, null);
                    Paint paint = new Paint();
                    paint.setColor(getResources().getColor(R.color.basic_color));
                    canvas.drawRoundRect(dst,8,8, paint);
                } else if (currentPosition == tabsLayout.getChildCount() - 1) {
//                    Bitmap bitmap = drawableToBitmap(slidingRightDrawable);
                    RectF dst = new RectF(slidingBlockLeft, slidingBlockTop, slidingBlockRight, slidingBlockBottom);
//                    canvas.drawBitmap(bitmap, null, dst, null);
                    Paint paint = new Paint();
                    paint.setColor(getResources().getColor(R.color.basic_color));
                    ...
                    Bitmap bitmap = drawableToBitmap(slidingMiddleDrawable);
                    RectF dst = new RectF(slidingBlockLeft, slidingBlockTop, slidingBlockRight, slidingBlockBottom);
                    canvas.drawBitmap(bitmap, null, dst, null);
                    Paint paint = new Paint();
                    paint.setColor(getResources().getColor(R.color.basic_color));
    ...
    public void setSlidingStrokeColor(int slidingStrokeColor) {
        this.slidingStrokeColor = slidingStrokeColor;
        invalidate();
    }

    public void setSlidingLeftDrawable(Drawable slidingLeftDrawable) {
        this.slidingLeftDrawable = slidingLeftDrawable;
        invalidate();
    }

    public void setSlidingMiddleDrawable(Drawable slidingMiddleDrawable) {
        this.slidingMiddleDrawable = slidingMiddleDrawable;
        invalidate();
    }

    public void setSlidingRightDrawable(Drawable slidingRightDrawable) {
        this.slidingRightDrawable = slidingRightDrawable;
        invalidate();
    }

    public void setSlidingStrokeDrawable(Drawable slidingStrokeDrawable) {
        this.slidingStrokeDrawable = slidingStrokeDrawable;
        invalidate();
    }
}